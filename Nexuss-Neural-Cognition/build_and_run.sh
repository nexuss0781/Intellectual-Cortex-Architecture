...
STAGE11_POST_TRAINING_ARTIFACT_DIR="${STAGE11_POST_TRAINING_ARTIFACT_DIR:-${ROOT_DIR}/artifacts/stage-11-post-training}"
STAGE12_ARTIFACT_DIR="${STAGE12_ARTIFACT_DIR:-${ROOT_DIR}/artifacts/stage-12-canonical}"
STAGE13_ARTIFACT_DIR="${STAGE13_ARTIFACT_DIR:-${ROOT_DIR}/artifacts/stage-13-preparation}"
STAGE14_ARTIFACT_DIR="${STAGE14_ARTIFACT_DIR:-${ROOT_DIR}/artifacts/stage-14-governance}"
SEED="${SEED:-424242}"

printf '%b\n' "${BOLD}[Nexuss] Clean reproducible validation${NC}"
printf '  root: %s\n  build: %s\n  artifacts: %s\n  seed: %s\n' "$ROOT_DIR" "$BUILD_DIR" "$ARTIFACT_DIR" "$SEED"

rm -rf "$BUILD_DIR" "$STAGE1_ARTIFACT_DIR" "$STAGE2_ARTIFACT_DIR" "$STAGE3_ARTIFACT_DIR" "$STAGE4_ARTIFACT_DIR" "$STAGE5_ARTIFACT_DIR" "$STAGE6_ARTIFACT_DIR" "$STAGE7_ARTIFACT_DIR" "$STAGE8_ARTIFACT_DIR" "$STAGE9_ARTIFACT_DIR" "$STAGE10_ARTIFACT_DIR" "$STAGE11_PREPARATION_ARTIFACT_DIR" "$STAGE11_POST_TRAINING_ARTIFACT_DIR" "$STAGE12_ARTIFACT_DIR" "$STAGE13_ARTIFACT_DIR" "$STAGE14_ARTIFACT_DIR"
...
"$BUILD_DIR/stage11_post_training_harness" --seed "$SEED" --artifact-dir "$STAGE11_POST_TRAINING_ARTIFACT_DIR" --repo-root "$ROOT_DIR" | tee "$STAGE11_POST_TRAINING_ARTIFACT_DIR/stage11_post_training_harness.txt"
"$BUILD_DIR/stage12_serving_harness" --seed "$SEED" --artifact-dir "$STAGE12_ARTIFACT_DIR" --repo-root "$ROOT_DIR" | tee "$STAGE12_ARTIFACT_DIR/stage12_serving_harness.txt"
( cd "$STAGE12_ARTIFACT_DIR" && find . -type f ! -name 'manifest.sha256' -print0 | sort -z | xargs -0 sha256sum > manifest.sha256 )
"$BUILD_DIR/stage13_preparation_harness" --artifact-dir "$STAGE13_ARTIFACT_DIR" --repo-root "$ROOT_DIR" | tee "$STAGE13_ARTIFACT_DIR/stage13_preparation_harness.txt"
( cd "$STAGE13_ARTIFACT_DIR" && find . -type f ! -name 'manifest.sha256' -print0 | sort -z | xargs -0 sha256sum > manifest.sha256 )
"$BUILD_DIR/stage14_governance_harness" --artifact-dir "$STAGE14_ARTIFACT_DIR" --repo-root "$ROOT_DIR" | tee "$STAGE14_ARTIFACT_DIR/stage14_governance_harness.txt"
( cd "$STAGE14_ARTIFACT_DIR" && find . -type f ! -name 'manifest.sha256' -print0 | sort -z | xargs -0 sha256sum > manifest.sha256 )
